...
static const  unsigned int color_lib[7]={	red,green,blue,gblue,yellow,rblue,orange,};
static game_obj_type game_obj[8][8]={0,};
static int score;
static lv_obj_t * screen1,* game_window,*refs_btn,*bgmap,*next_btn,*pre_btn,*step_lable,*exit_btn,*coin,*score_lable;
		  ...
		  if( (movex==0&&movey==0)||(movex==movey)||(movex==-movey) )return;
			...
			if( (movex<0&&movey>0&&movex>-movey) || (movex>0&&movey>0&&movex< movey) )direction=down;
			 ...
			 if(direction==left)
			 {
				 ...
				 else{exchange_obj(stage_data,&game_obj[stage_data->y][stage_data->x-1]);}
			 }
			 
		  if(direction==right)
			 {
				 if(stage_data->x==7){return;}
				 else
...
static void exchange_done_cb(lv_anim_t * a)
{	
		if(same_color_check()){same_color_flash();}		
		else{add_all_clickable();}			
}
...
static int same_color_check()
{
	...
	for(i=0;i<8;i++)
	{
		for(j=0;j<6;j++)
		{
			if(game_obj[j][i].color_index==game_obj[j+1][i].color_index&&game_obj[j][i].color_index==game_obj[j+2][i].color_index)
			{
				game_obj[j][i].alive=0;game_obj[j+1][i].alive=0;game_obj[j+2][i].alive=0;m++;
			}			
		}	
	}
	...
	for(j=0;j<8;j++)
	{
		for(i=0;i<6;i++)
		{
			if(game_obj[j][i].color_index==game_obj[j][i+1].color_index&&game_obj[j][i].color_index==game_obj[j][i+2].color_index)
			{
				game_obj[j][i].alive=0;game_obj[j][i+1].alive=0;game_obj[j][i+2].alive=0;m++;
			}			
		}	
	}	
	return m;
...
{	
	...
	for(j=0;j<8;j++)
	{
		for(i=0;i<8;i++)
		{
			if(lv_obj_is_valid(game_obj[j][i].obj))game_obj[j][i].obj->user_data=&game_obj[j][i];
		}
	}	
}
...
static void map_del_all()
{
	int i,j,k;
		for(j=0;j<8;j++)
		{
			for(i=0;i<8;i++)
			{
					if(lv_obj_is_valid(game_obj[j][i].obj))lv_obj_del(game_obj[j][i].obj);
			}
		}	
}	
...
static void clear_all_clickable()
{
		...
		for(j=0;j<8;j++)
		{
			for(i=0;i<8;i++)
			{
					if(lv_obj_is_valid(game_obj[j][i].obj))lv_obj_clear_flag(game_obj[j][i].obj,LV_OBJ_FLAG_CLICKABLE);
			}
		}				
}
...
static void add_all_clickable()
{
		...
		for(j=0;j<8;j++)
		{
			for(i=0;i<8;i++)
			{
					if(lv_obj_is_valid(game_obj[j][i].obj))lv_obj_add_flag(game_obj[j][i].obj,LV_OBJ_FLAG_CLICKABLE);
			}
		}				
}
