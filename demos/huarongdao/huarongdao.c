...
const static game_obj_type game_stage_obj_lib[][10]={
																							 ...
																							 0,0,2,hor,								0,2,2,hor,
																							 0,0,3,little, 0,2,3,big, 
																							 0,0,4,little,						
	
	
																							0,0,0,ver,   0,1,0,ver,  0,2,0,ver,0,3,0,little,0,3,1,little,
																							 0,0,2,hor,		0,0,3,hor,0,2,2,big,
																							 0,2,4,little,  0,3,4,little,
																							 	
	
																								0,0,0,ver, 0,1,0,ver, 0,2,0,ver, 0,3,0,ver,
																								0,0,2,ver, 0,1,2,big,				     
	                                              0,0,4,little, 0,1,4,little, 0,2,4,little, 0,3,4,little,
	 
																								0,0,0,hor, 0,2,0,hor,
																								0,0,1,hor, 0,2,1,hor,
																								0,0,2,big,  0,2,2,little, 0,3,2,little,0,2,3,little, 0,3,3,little,
	                                              0,0,4,hor,
	                                            
	
	                                              0,0,0,hor, 0,2,0,hor,
																								0,0,1,big, 0,2,1,hor, 0,2,2,hor,				     
	                                              0,0,3,hor,0,2,3,little, 0,3,3,little,  
																								0,0,4,little, 0,3,4,little,	
	

																								0,0,0,big, 0,2,0,hor, 0,2,1,hor,
																								0,0,2,hor, 0,2,2,hor,				     
	                                              0,0,3,little, 0,1,3,little, 0,2,3,hor, 
																								0,0,4,little, 0,3,4,little,
		
																								0,0,0,hor,   0,0,1,hor,0,2,0,big,
																								0,0,2,hor,   0,2,2,hor,   
																								0,0,3,ver,   0,2,3,little, 0,3,3,little,  0,2,4,little, 0,3,4,little,
																								
																								0,0,0,ver, 0,1,0,big,  0,3,0,ver,
																								0,0,2,ver, 0,1,2,little, 0,2,2,little, 0,3,2,ver,				     
																													 0,1,3,little, 0,2,3,little,
																								           0,1,4,hor,
																								
																								
																								0,0,0,ver, 0,1,0,little, 0,2,0,little, 0,3,0,ver,
																								                 0,1,1,big,
																								0,0,2,little,                      0,3,2,little, 				     
																								0,0,3,ver,          0,1,3,hor,					0,1,4,hor,																	
																								
																								
		
																								0,0,0,hor,   0,2,0,little,   0,3,0,little,
																								0,0,1,ver,   0,1,1,big,   0,3,1,ver,
																								0,0,3,ver,   0,1,3,little, 0,1,4,little, 0,2,3,ver, 
		
																								0,0,0,little,   0,1,0,hor,   0,3,0,ver,
																								0,0,1,ver,   0,1,1,hor,   
																								0,1,2,little,   0,2,2,big, 0,0,3,hor, 0,2,4,little, 0,3,4,little, 
	

                                               0,0,0,ver,   0,1,0,little,   0,1,1,little, 0,2,0,ver, 0,3,0,ver,
																							 0,0,2,ver,   0,1,2,little,   0,1,3,little,  0,2,2,big,
	                                             0,2,4,hor, 

	
																							 0,0,0,ver,   0,1,0,big,  0,3,0,ver,
																							 0,0,2,ver,		0,1,2,hor,	0,3,2,ver,
																							 0,0,4,little,   0,1,3,little,0,2,3,little,0,3,4,little,
																							 						
																							
																							 0,0,0,ver,   0,1,0,big,  0,3,0,ver,
																							 0,0,2,hor,								0,2,2,hor,
																							 0,0,3,little, 0,1,3,hor,  0,3,3,little,
																							 0,0,4,little,						0,3,4,little,
																							 
																							 
																							 0,0,0,ver,   0,1,0,big,  0,3,0,little,0,3,1,little,
																							 0,1,2,hor,								
...
static game_obj_type game_stage_obj[10]={0,};
static uint8_t game_map[5][4]={0,};
...
void huarongdao()
{
		...
		lv_obj_set_size(game_window,lv_pct(40),lv_pct(80));
...
static void next_stage_cb(lv_event_t * e)
{
	if(current_stage==max_stage-1)return;
	lv_obj_clean(game_window);
	current_stage++;
	game_stage_init(0);
...
static void game_stage_init(lv_timer_t * t)
{
	step_count=0;lv_label_set_text_fmt(step_lable, "STEP:%d",step_count);
	clear_map();
		
  lv_memcpy(game_stage_obj, (void *)((int)game_stage_obj_lib+sizeof(game_stage_obj)*current_stage), sizeof(game_stage_obj));
	
	for(int i=0;i<10;i++)
	{
		game_stage_obj[i].obj=lv_btn_create(game_window);
		lv_obj_set_size(game_stage_obj[i].obj,game_stage_obj[i].obj_type==little||game_stage_obj[i].obj_type==ver?lv_pct(25):lv_pct(50),game_stage_obj[i].obj_type==big||game_stage_obj[i].obj_type==ver?lv_pct(40):lv_pct(20));
		lv_obj_set_pos(game_stage_obj[i].obj,lv_pct((game_stage_obj[i].x)*25),lv_pct((game_stage_obj[i].y)*20));
		lv_obj_set_style_bg_color(game_stage_obj[i].obj,lv_color_hex(game_stage_obj[i].obj_type==little?0xffa200:game_stage_obj[i].obj_type==big?0xff5555:game_stage_obj[i].obj_type==ver?0x6666ff:0x44bb44), LV_PART_MAIN);
			...
			if( (movex<0&&movey>0&&movex>-movey) || (movex>0&&movey>0&&movex< movey) )direction=down;
			 ...
			 if(direction==up)
			 {
				 ...
				 if(stage_data->obj_type==big)
				 {
					 ...
					 {
						 game_map[stage_data->y-1][stage_data->x]=1;game_map[stage_data->y-1][(stage_data->x)+1]=1;
						 game_map[stage_data->y+1][stage_data->x]=0;game_map[stage_data->y+1][(stage_data->x)+1]=0;
						 stage_data->y--;step_count++;lv_label_set_text_fmt(step_lable, "STEP:%d",step_count);
						 lv_obj_set_pos(stage_data->obj,lv_pct((stage_data->x)*25),lv_pct((stage_data->y)*20));
					 }
				 }
				 ...
				 if(stage_data->obj_type==hor)
				 {
					 ...
					 {
						 game_map[stage_data->y-1][stage_data->x]=1;game_map[stage_data->y-1][(stage_data->x)+1]=1;
						 game_map[stage_data->y  ][stage_data->x]=0;game_map[stage_data->y  ][(stage_data->x)+1]=0;
						 stage_data->y--;step_count++;lv_label_set_text_fmt(step_lable, "STEP:%d",step_count);
						 lv_obj_set_pos(stage_data->obj,lv_pct((stage_data->x)*25),lv_pct((stage_data->y)*20));
					 }
				 }
			 ...
			 if(direction==down)
			 {
				 ...
				 if(stage_data->obj_type==big)
				 {
					 if(stage_data->y==3)return;
					 
					 if(game_map[stage_data->y+2][stage_data->x]==0&&game_map[stage_data->y+2][(stage_data->x)+1]==0)
					 {
						 game_map[stage_data->y+2][stage_data->x]=1;game_map[stage_data->y+2][(stage_data->x)+1]=1;
						 game_map[stage_data->y  ][stage_data->x]=0;game_map[stage_data->y  ][(stage_data->x)+1]=0;
						 stage_data->y++;step_count++;lv_label_set_text_fmt(step_lable, "STEP:%d",step_count);
						 lv_obj_set_pos(stage_data->obj,lv_pct((stage_data->x)*25),lv_pct((stage_data->y)*20));
					 }
			 ...
			 if(direction==left)
			 {
				 ...
				 if(stage_data->obj_type==little)
				 {
					 if(stage_data->x==0)return;
					 
					 if(game_map[stage_data->y][stage_data->x-1]==0)
					 {
						 game_map[stage_data->y  ][stage_data->x-1]=1;
						 game_map[stage_data->y  ][stage_data->x  ]=0;
						 stage_data->x--;step_count++;lv_label_set_text_fmt(step_lable, "STEP:%d",step_count);
						 lv_obj_set_pos(stage_data->obj,lv_pct((stage_data->x)*25),lv_pct((stage_data->y)*20));
					 }
				 }
				 ...
				 if(stage_data->obj_type==big)
				 {
					 ...
					 {
						 game_map[stage_data->y][stage_data->x-1]=1;game_map[stage_data->y+1][(stage_data->x)-1]=1;
						 game_map[stage_data->y][stage_data->x+1]=0;game_map[stage_data->y+1][(stage_data->x)+1]=0;
						 stage_data->x--;step_count++;lv_label_set_text_fmt(step_lable, "STEP:%d",step_count);
						 lv_obj_set_pos(stage_data->obj,lv_pct((stage_data->x)*25),lv_pct((stage_data->y)*20));
					 }
				 }
				 ...
				 if(stage_data->obj_type==ver)
				 {
					 ...
					 {
						 game_map[stage_data->y][stage_data->x-1]=1;game_map[stage_data->y+1][stage_data->x-1]=1;
						 game_map[stage_data->y][stage_data->x  ]=0;game_map[stage_data->y+1][stage_data->x  ]=0;
						 stage_data->x--;step_count++;lv_label_set_text_fmt(step_lable, "STEP:%d",step_count);
						 lv_obj_set_pos(stage_data->obj,lv_pct((stage_data->x)*25),lv_pct((stage_data->y)*20));
					 }
				 }				 
			 ...
			 if(direction==right)
			 {
				 ...
				 if(stage_data->obj_type==big)
				 {
					 if(stage_data->x==2)return;
					 
					 if(game_map[stage_data->y][stage_data->x+2]==0&&game_map[stage_data->y+1][(stage_data->x)+2]==0)
					 {
						 game_map[stage_data->y][stage_data->x+2]=1;game_map[stage_data->y+1][(stage_data->x)+2]=1;
						 game_map[stage_data->y][stage_data->x  ]=0;game_map[stage_data->y+1][(stage_data->x)  ]=0;
						 stage_data->x++;step_count++;lv_label_set_text_fmt(step_lable, "STEP:%d",step_count);
						 lv_obj_set_pos(stage_data->obj,lv_pct((stage_data->x)*25),lv_pct((stage_data->y)*20));
					 }
		     ...
		     lv_label_set_text(clear_lable, "STAGE CLEAR");
				 ...
				 if(current_stage<max_stage-1){current_stage++;}
				 stage_clear();
			 }
