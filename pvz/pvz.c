...
static zb_type  zb_matrix[max_zb_count]={0,};
static char map_flag[5][9]={0,};
static plant_type  wogua[max_quantity]={0,};
static plant_type  add_zidan_timer[max_quantity]={0,};
static plant_type  sunflower[max_quantity]={0,};
static plant_type  wandouflower[max_quantity]={0,};
static plant_type  jiguangdou[max_quantity]={0,};
static plant_type  dici[max_quantity]={0,};
static shine_type  shine[max_quantity]={0,};
static zidan_type  zidan[max_quantity]={0,};
static zidan_type  little_car[5]={0,};
...
const static zb_class_type zb_class[]={ Normalzb,(void*)&zb1,20,150,0,zb_dead_anim,Normalzb_start_anim_cb,
...
static card_btn_type card_btn[]={	0,(void *)&sun_btn_img,sunflower_card,0,btn_cliked_cb,sunflower_price,sunflower_creat_cb,
...
static void game_start(lv_event_t * e)
{
  int i;
		...
		for(i=0;i<sizeof(card_btn)/sizeof(card_btn_type);i++)
		{
            ...
            lv_obj_set_style_text_font(price_lable,&lv_font_montserrat_12,0);
...
static void zb_dead_anim(zb_type * xzb)
{
		...
		if(xzb->zb_class->zb_kind==Normalzb){lv_anim_del(xzb->zb,zb_set_angle_cb);}
		if(xzb->zb_class->zb_kind==jumpzb){lv_anim_del(xzb->zb,jumpzb_jump_cb);}
...
static void cherry_delete_cb(lv_anim_t * a)
{
	int i;
	  ...
	  lv_anim_set_ready_cb(&a1,boom_delete_cb);
		...
		for(i=0;i<max_zb_count;i++)
	    {if(zb_matrix[i].blood>0&&lv_obj_get_x(zb_matrix[i].zb)-x<80&&x-lv_obj_get_x(zb_matrix[i].zb)<80&&lv_obj_get_y(zb_matrix[i].zb)-y<80&&y-lv_obj_get_y(zb_matrix[i].zb)<80)
				 {
					zb_matrix[i].blood=0;
					zb_matrix[i].zb_class->zb_del_cb(&zb_matrix[i]);
                                     ...
                                     if((zb_matrix[j].x-zidan[i].x<15)&&(zidan[i].x-zb_matrix[j].x<10))
                                          {
                                            ...
                                            lv_obj_set_pos(zidan_split,lv_obj_get_x(zidan[i].zidan)-8,lv_obj_get_y(zidan[i].zidan)-8);
                            ...
                            for(i=0;i<max_zb_count;i++)
                             {
                                if(zb_matrix[i].blood>0)
                                {
                                    if((zb_matrix[i].y==wogua->y)&&(zb_matrix[i].x-wogua->x*49<80)&&(zb_matrix[i].x-wogua->x*49>50))
                                     {
                                        wogua->blood=0;
                                        zb_matrix[i].blood=0;
                     ...
                     if(dici->blood>0)
						{
                            ...
                            for(i=0;i<max_zb_count;i++)
                             {
                                if(zb_matrix[i].blood>0&&(zb_matrix[i].zb_class->zb_kind==Normalzb||zb_matrix[i].zb_class->zb_kind==carzb))
                                {
                                    if((zb_matrix[i].y==dici->y)&&(zb_matrix[i].x-dici->x*49<50)&&(zb_matrix[i].x-dici->x*49>-50))
                                     {
...
for(int i=0;i<sizeof(card_btn)/sizeof(card_btn_type);i++)
	{
		if(sun_score>=card_btn[i].needscore)
		{
			...
			lv_obj_set_style_text_color(lv_obj_get_child(card_btn[i].card_btn,0),lv_color_hex(0xffffff), LV_PART_MAIN);
		}
		else
        {
            lv_obj_set_style_img_recolor_opa(card_btn[i].card_btn,200,LV_PART_MAIN);
			lv_obj_clear_flag(card_btn[i].card_btn, LV_OBJ_FLAG_CLICKABLE);
			lv_obj_set_style_text_color(lv_obj_get_child(card_btn[i].card_btn,0),lv_color_hex(0x555555), LV_PART_MAIN);
...
static void btn_cliked_cb(lv_event_t * e)
{
	...
	for(int i=0;i<sizeof(card_btn)/sizeof(card_btn_type);i++)
	{
		if(card_btn[i].card_btn==e->target)
		{
...
static void map_click_cb(lv_event_t * e)
{
	...
	if(chanzi_btn_select)
		...
		for(i=0;i<sizeof(card_btn)/sizeof(card_btn_type);i++)
				{
					 if(card_btn[i].ispushed==1){break;}
				}
...
static void add_jiguang_cb(lv_timer_t * t)
{
		static  lv_point_t  jiguang_point[max_quantity][2]={0,};
    ...
    lv_obj_t * jiguang_track1=lv_line_create(map1);
	  ...
	  jiguang_track2->user_data=jiguang_track1;
	    ...
	    point_count++;
	    if(point_count==max_quantity)point_count=0;
       ...
       for(i=0;i<max_zb_count;i++)
				{
					if(zb_matrix[i].blood>0&&y==zb_matrix[i].y)
					 {
						zb_matrix[i].blood--;
						if(zb_matrix[i].blood==0){zb_matrix[i].zb_class->zb_del_cb(&zb_matrix[i]);}
						else
							{
                                lv_anim_t a1;
                                lv_anim_init(&a1);
